...
public class EntityManager
{
	...
	private static int m_controlledEntity; // ID of the entity that the player controls
	...
	// Returns state for the controlled entity, or null if it has not changed since last call
	public static JSONObject getControlledEntityStateUpdate()
	{
		if(m_entities.get(m_controlledEntity).getStateChanged())
		{
			return m_entities.get(m_controlledEntity).getJSON();
		}
		else
		{
			return null;
	...
	public static void updateState(JSONObject serverState)
	{
		...
		{
			entityState = (JSONObject)serverState.get(String.valueOf(i));
			if(entityState != null)
