...
public class ClientConnections
{
	...
	public void sendState(JSONObject state)
	{
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			{
				m_UDPSenders[i].send((JSONObject)state.clone()); // Shallow copy should be fine
			}
		}
	}
	
	public void sendScore(JSONObject score)
	{
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			if(m_TCPSenders[i] != null)
			{
				m_TCPSenders[i].send(score);
			}
		}
	}
	...
	public JSONObject getUpdatedStates()
	{
		...
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			if(m_UDPReceivers[i] != null)
			{
				states.put(String.valueOf(i+1), m_UDPReceivers[i].getState()); // i+1 because ball is first entity
			}
		}
		return states;
	}
	...
	private void addClient(Socket TCPSocket)
	{
		try
		{
			...
			m_UDPReceivers[index] = new UDPReceiver(UDPSocket, index, token);
	...
	private class UDPReceiver implements Runnable
	{
		...
		public UDPReceiver(DatagramSocket socket, int index, String token)
		{
			m_socket = socket;
			m_index = index;
		...
		public void run()
		{
			...
			while(m_run[m_index])
			{
				try
				{
					...
					if(!m_token.equals((String)received.get("tkn")))
